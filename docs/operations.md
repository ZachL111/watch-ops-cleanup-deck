...
`watch-ops-cleanup-deck` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for cleanup scenarios with node-edge fixtures, cycle and reachability reports, and local-only command execution.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
